...
from mongoengine.fields import (StringField,  URLField,
                                EmailField,  IntField,  LongField, FloatField,
                                DecimalField,  BooleanField,  DateTimeField,
                                ImageField,)
from mongonaut.fields import AdminStringField, AdminUnsignedIntField, AdminImageURLField

from .filters import (Filter, CharFilter, BooleanFilter,
    ChoiceFilter, DateFilter, ModelChoiceFilter,
    ModelMultipleChoiceFilter, NumberFilter, EmailFilter)
...
class BaseFilterSet(object):
    ...
    def get_default_data(self):
        ...
        for field, filter in six.iteritems(self.filters):
            if filter.default:
                self.default_data[field] = filter.default
    ...
    def form(self):
        if not hasattr(self, '_form'):
            ...
            if self.is_bound:
                self._form = Form(self.data, initial=self.default_data, prefix=self.form_prefix)
            else:
                self._form = Form(initial=self.default_data, prefix=self.form_prefix)
        search_fields = self._meta.fields
        for item in search_fields:
            field = getattr(self._meta.model, item)
            self.set_form_field(field, item)
        return self._form

    def set_form_field(self, model_field, field_key):
        if model_field.choices:
            self._form.fields[field_key].choices = model_field.choices

        CHECK_ATTRS = {'help_text': 'help_text',
                       'name': 'name'}

        for key in CHECK_ATTRS.keys():
            if hasattr(model_field, key):
                value = getattr(model_field, key)
                setattr(self._form.fields[field_key], key, value)
    ...
    def filter_for_field(cls, f, name):
        ...
        if data is None:
            ...
            for class_ in inspect.getmro(f.__class__):
