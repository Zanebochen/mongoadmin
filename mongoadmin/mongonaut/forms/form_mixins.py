...
class MongoForm(forms.Form):
    ...
    def media(self):
        ...
        # Add some media that validation need.
        if hasattr(self, 'validation_media'):
            media += self.validation_media
        return media
...
class MongoModelFormBaseMixin(object):
    ...
    def __init__(self, model, instance=None, form_post_data=None, files=None):
        """
        Params:
        ...
        self.form = MongoForm(files=files)
    ...
    def set_form_field(self, widget, model_field, field_key, default_value):
        ...
        if field_class is IntChoiceField:
            # 单独处理AdminIntSelectField
            # 从mongo中取key时转为value, 此时需将value转为key, 供前端使用.
            # select控件不能用(value, value), 因为mongofield会有self.choices校验.
            default_value = model_field.to_mongo(default_value)
    ...
    def get_field_value(self, field_key):
        ...
        def get_value(document, field_key):
            ...
            if key_array_digit is not None and len(new_key_array) > 0:
                ...
                if callable(transform_method) and return_data:
                    return_data = transform_method(return_data)
